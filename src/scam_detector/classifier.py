...
async def calculate_keyword_score(message: str) -> Tuple[float, List[str]]:
    ...
    category_bonus = min(
        len(matched_categories) * 0.1 + (category_severity_total / 50), 0.4
    )
...
async def calculate_pattern_score(message: str) -> float:
    ...
    if "http://" in message_lower or "https://" in message_lower:
        score += 0.2
        if any(
            sus in message_lower
            for sus in [".xyz", ".tk", ".top", "bit.ly", "tinyurl", "short"]
        ):
            score += 0.15
    ...
    if any(
        ...
        for phrase in ["video call", "skype", "zoom call", "stay on call"]
    ):
        score += 0.25
